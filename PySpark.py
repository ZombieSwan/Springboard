...
result = spark.sql("""
    ...
    GROUP BY 
        education
    ORDER BY 
        bachelor_rate DESC, total_count DESC
""")
...
from pyspark.sql.functions import when, col, mean, desc, round, trim

# wrangle the data a bit
df_result = df_adult.select(
    df_adult['education'],
    when(trim(col('marital_status')) == 'Never-married', 1).otherwise(0).alias('is_bachelor')
)

# Group by 'education', calculate the mean 'bachelor_rate', and order the results
df_result = df_result.groupBy('education') \
                     .agg(round(mean('is_bachelor'), 2).alias('bachelor_rate')) \
...
gbParamGrid = (ParamGridBuilder()
             ...
             .addGrid(gb.stepSize, [0.1, 0.3])
             .addGrid(gb.minInstancesPerNode, [1, 2])
             .build())
...
# Run cross validations on the GBT model
...
print("Best Min Instances Per Node: ", gbCvModel.bestModel._java_obj.getMinInstancesPerNode())
...
# Assuming you have a pipeline variable named 'pipeline', find the VectorAssembler stage.
# If you don't have a pipeline variable, you should know the exact name of the VectorAssembler output column (usually named 'features').
assembler = [s for s in pipeline.getStages() if isinstance(s, VectorAssembler)][0]

# Get the input columns from the VectorAssembler stage.
input_features = assembler.getInputCols()

# Ensure that the coefficients are in a list of floats
coefficients = [float(c) for c in lrCvModel.bestModel.coefficients]

# Zip the input columns with the weights from the Logistic Regression model.
feature_coefficients = list(zip(input_features, coefficients))

# Convert to DataFrame.
features_and_weights_df = spark.createDataFrame(feature_coefficients, ["Feature_Name", "Feature_Coefficient"])

# Show the DataFrame.
features_and_weights_df.show()
...
# Extract the feature importances from the best GBT model
gbt_feature_importances = gbCvModel.bestModel.featureImportances.toArray()

# Check the lengths
print(f"Number of input_features: {len(input_features)}")
print(f"Number of gbt_feature_importances: {len(gbt_feature_importances)}")

# Instead of asserting that the lengths match, let's just use the minimum of the two lengths to safely zip the two lists:
min_length = min(len(input_features), len(gbt_feature_importances))

# Zip the input feature names with their importances up to the minimum length
gbt_feature_importance_tuples = list(zip(input_features[:min_length], gbt_feature_importances[:min_length]))

# Convert numpy.float64 to python's native float type
gbt_feature_importance_tuples = [(f, float(imp)) for f, imp in zip(input_features[:min_length], gbt_feature_importances[:min_length])]

# Convert the list of tuples into a DataFrame
gbt_features_and_importance_df = spark.createDataFrame(gbt_feature_importance_tuples, ["Feature_Name", "Feature_Importance"])

# Sort the DataFrame based on feature importance in descending order and display it
gbt_features_and_importance_df.orderBy("Feature_Importance", ascending=False).show()



# COMMAND ----------

print("Number of features from VectorAssembler:", len(input_features))
print("Number of features from GBT Model:", len(gbCvModel.bestModel.featureImportances))
...
# Since we've already defined the assembler to combine features, 
# we can reuse its input columns to map feature importances.
input_features = assembler.getInputCols()
importance_values = [float(i) for i in rfFeatureImportance]

feature_importances = list(zip(input_features, importance_values))
feature_importances_df = spark.createDataFrame(feature_importances, ["Feature_Name", "Feature_Importance"])
